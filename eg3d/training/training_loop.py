...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    grid_indices = [i for i in range(28)]
...
def save_image_grid(img, fname, drange, grid_size, wandb=False):
    lo, hi = drange
...
def training_loop(
    ...
    run = wandb.init(
        project='eg3d',
        config={
            "training_set": training_set_kwargs,
    ...
    if rank == 0:
        ...
        img, _, _, _, _ = misc.print_module_summary(G, [z, c, smpl])
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            wandb.log({'fakes': wandb.Image(save_image_grid(images, os.path.join(run_dir, ''), drange=[-1,1], grid_size=grid_size, wandb=True), caption='fakes'), 
                       'fakes_raw': wandb.Image(save_image_grid(images_raw, os.path.join(run_dir, ''), drange=[-1,1], grid_size=grid_size, wandb=True), caption='fakes_raw'), 
                       'fakes_depth': wandb.Image(save_image_grid(images_depth, os.path.join(run_dir, ''), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size, wandb=True), caption='fakes_depth')})
            ...
            wandb.log({'fakes_cano': wandb.Image(save_image_grid(images, os.path.join(run_dir, ''), drange=[-1,1], grid_size=grid_size, wandb=True), caption='fakes_cano'), 
                       'fakes_cano_raw': wandb.Image(save_image_grid(images_raw, os.path.join(run_dir, ''), drange=[-1,1], grid_size=grid_size, wandb=True), caption='fakes_cano_raw'), 
                       'fakes_cano_depth': wandb.Image(save_image_grid(images_depth, os.path.join(run_dir, ''), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size, wandb=True), caption='fakes_cano_depth')})
