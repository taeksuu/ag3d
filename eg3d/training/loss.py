...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, smpl, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        gen_output, deformer_weight_diffs, eikonal, zero_delta_sdf, bbox = self.G.synthesis(ws, c, smpl, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
        return gen_output, ws, deformer_weight_diffs, eikonal, zero_delta_sdf, bbox
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_smpl, gen_z, gen_c, gen_smpl, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, deformer_weight_diffs, eikonal, zero_delta_sdf, bbox = self.run_G(gen_z, gen_c, gen_smpl, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                ...
                training_stats.report('Loss/bbox', bbox)
                wandb.log({'deformer': deformer_weight_diffs, 'eikonal': eikonal, 'zero_delta_sdf': zero_delta_sdf, 'bbox': bbox})
                loss_Gmain = torch.nn.functional.softplus(-gen_logits).mean() + deformer_weight_diffs + eikonal + zero_delta_sdf + bbox
                training_stats.report('Loss/G/loss', loss_Gmain)
                wandb.log({'G_loss': loss_Gmain})
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws, deformer_weight_diffs, eikonal, zero_delta_sdf, bbox = self.run_G(gen_z, gen_c, gen_smpl, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
                ...
                training_stats.report('Loss/zero_delta_sdf/', zero_delta_sdf)
                training_stats.report('Loss/bbox/', bbox)
                wandb.log({'scores/fake': torch.nn.functional.softplus(gen_logits).mean(), 'deformer': deformer_weight_diffs, 'eikonal': eikonal, 'zero_delta_sdf': zero_delta_sdf, 'bbox': bbox})
                loss_Dgen = torch.nn.functional.softplus(gen_logits).mean() + deformer_weight_diffs + eikonal + zero_delta_sdf + bbox
                wandb.log({'loss_Dgen': loss_Dgen})
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                wandb_scores_real = torch.nn.functional.softplus(-real_logits).mean()
                wandb.log({'scores/real': wandb_scores_real})

                loss_Dreal = 0
                if phase in ['Dmain', 'Dboth']:
                    loss_Dreal = torch.nn.functional.softplus(-real_logits)
                    training_stats.report('Loss/D/loss', loss_Dgen + loss_Dreal)
                    wandb.log({'D_loss': loss_Dgen + loss_Dreal.mean().item()})
                ...
                if phase in ['Dreg', 'Dboth']:
                    ...
                    wandb.log({'D_reg': loss_Dr1.mean().item(), 'r1_penalty': r1_penalty.mean().item()})
