...
def query_weights_smpl(x, smpl_verts, smpl_weights):
    ...
    distance_batch, index_batch, neighbor_points  = ops.knn_points(x,smpl_verts,K=10,return_nn=True)
    ...
    skinning_weights = (smpl_weights[0][index_batch.view(-1)] * ws.view(-1, 1)).reshape(ws.shape[0], 10, -1).sum(1)
